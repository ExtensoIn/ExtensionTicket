                ...
                <ul className='hidden sm:flex gap-2 font-DMSans items-center'>
                    {linkList.map((link: { name: string, url: string }) => (
                        link.name === 'Login' ? !isAuthenticated?.data ?
                            <span key={link.url} onClick={() => login()} className='hover:bg-white hover:text-purple-800 border-1 border-white rounded-[2rem] px-4 py-2'>Login</span> :
                            <span key={link.url} onClick={() => logout()} className='hover:bg-white hover:text-purple-800 border-1 border-white rounded-[2rem] px-4 py-2'>Logout</span> :
