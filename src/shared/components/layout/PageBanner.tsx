...
const ImgBannerType = {
    normal: 'w-48 md:w-64 lg:w-80 ',
    large: 'w-48 sm:w-64 md:w-72 smd:w-96 lg:w-[28rem]',
}

function PageBanner({ imgSrc, children, title, description, imgMaxWidth = 'normal' }: PageBannerProps) {
    return (
        <section className='w-full flex flex-col gap-4 smd:flex-row justify-between xl:gap-24 xl:justify-center items-center px-10 md:px-20 lg:px-40'>
            ...
            <span className='max-w-96 flex flex-col gap-6 text-center smd:text-left'>
                ...
                <p className='font-AbhayaLibre text-lg'>{description}</p>
                {children}
            </span>
        </section>
    )
